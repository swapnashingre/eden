...
function s3_gis_ac_search_selected(a){var b;for(b=0;5>=b;b++)$("#gis_location_L"+b+"_search").val(""),$("#gis_location_L"+b+"_label__row").hide(),$("#gis_location_L"+b+"_search__row").hide();var c=a.path,f=a.parent;if(c){var d=c.slice(0,c.toString().length+1).split("/"),e=d.length-1;if(0==e)$("#gis_location_search_throbber").hide(),$("#gis_location_search_select-btn").removeClass("hide").show();else for(b=0;b<=e;b++)b==e-1?s3_gis_search_hierarchy(d[b],!1,!0):s3_gis_search_hierarchy(d[b],!1,!1)}else f&&
...
$("#gis_location_lon_search").val(""),$("#gis_location_lon_search__row").hide()));$("body").data("id",a.id);!c&&!f&&$("#gis_location_search_select-btn").removeClass("hide").show()}
function s3_gis_lat_lon_converter(){function a(a){return!isNaN(parseFloat(a))&&isFinite(a)}function b(a){return a.parents(".gis_coord_wrap").eq(0)}function c(a){return function(c){a.val(c||"");"undefined"==typeof c&&a.focus()}}function f(a){var c=Math.abs(a),c=60*(c-parseInt(c));if(1E-10>Math.abs(c-Math.round(c)))c=Math.round(c),b=0;else{var b=60*(c-parseInt(c));1E-10>Math.abs(b-Math.round(b))&&(b=Math.round(b))}return{d:parseInt(a),m:parseInt(c),s:b}}function d(b){var d=$(".degrees",b).val()||0,
e=$(".minutes",b).val()||0,i=$(".seconds",b).val()||0,j=c($(".degrees",b)),k=c($(".minutes",b)),l=c($(".seconds",b)),p=c($(".decimal",b)),b="gis_location_lat"==$(".decimal",b).attr("id");a(d)?(d=Number(d),Math.abs(d)>(b?90:180)?(alert(h.degrees[b?"lat":"lon"]),j()):a(e)?(e=Math.abs(e),60<e?(alert(h.minutes),k()):a(i)?(i=Math.abs(i),60<=i?(alert(h.seconds),l()):(d=(0>d?-1:1)*(Math.abs(d)+e/60+i/3600),Math.abs(d)>(b?90:180)?alert(h.decimal[b?"lat":"lon"]):(e=f(d),p(""+d),j(e.d||"0"),k(e.m||"0"),l(e.s||
"0")))):(alert(g.seconds),l())):(alert(g.minutes),k())):(alert(g.degrees),j())}function e(c){var b=$(".decimal",c),d=b.val(),e="gis_location_lat"==$(".decimal",c).attr("id");""!=d&&(a(d)?(d=Number(d),Math.abs(d)>(e?90:180)?(alert(h.decimal[e?"lat":"lon"]),b.focus()):(b=f(d),$(".degrees",c).val(b.d||"0"),$(".minutes",c).val(b.m||"0"),$(".seconds",c).val(b.s||"0"))):(alert(g.decimal),b.val("").focus()))}var g=S3.i18n.gis_only_numbers,h=S3.i18n.gis_range_error;$(".gis_coord_dms input").blur(function(){d(b($(this)))}).keypress(function(a){13==
...
function s3_gis_search_hierarchy(a,b,c){$("#gis_location_search_throbber").removeClass("hide").show();$.ajax({async:!0,url:S3.gis.url+"/"+a+".json",dataType:"json",success:function(f){if(f[0].id==a){var d=f[0].name,e=f[0].level,f=f[0].parent;$("body").data(e,a);$("#gis_location_"+e+"_search").val(d);$("#gis_location_"+e+"_label__row").removeClass("hide").show();$("#gis_location_"+e+"_search__row").removeClass("hide").show();b&&f?s3_gis_search_hierarchy(f,!0,!0):c&&($("#gis_location_search_throbber").hide(),
...
function s3_gis_l0_select(){var a=$("#gis_location_L0").val(),b=S3.gis.url.replace("gis/location","gis/l0")+"/"+a;$.ajax({async:!0,url:b,dataType:"json",success:function(b){if(b.id==a){S3.gis.country=b.code;for(level=1;6>level;level++){var f="L"+level;if(b[f]){$("#gis_location_"+f+"_label__row label").text(b[f]+":");s3_gis_show_level(level);var d=$("#gis_location_"+f+"__row div.tooltip"),e=d.attr("title").split("|");d.attr("title",b[f]+"|"+e[1]+"|"+e[2]);d.cluetip({activation:"hover",sticky:!1,splitTitle:"|"})}else s3_gis_hide_level(level)}if(!S3.gis.no_map&&
(f=$("#gis_location_lat").val(),d=$("#gis_location_lon").val(),!f&&!d)){var f=b.lon_min,d=b.lat_min,e=b.lon_max,g=b.lat_max;f&&d&&e&&g?s3_gis_zoomMap(f,d,e,g):(f=b.lat,d=b.lon,f&&d&&(b=new OpenLayers.LonLat(d,f),b.transform(S3.gis.proj4326,S3.gis.projection_current),S3.gis.mapWin.rendered?map.setCenter(b):S3.gis.mapPanel.center=b))}}}});$("#gis_location_L1_ac").autocomplete("option","source",s3_gis_ac_set_source(1));$("#gis_location_L2_ac").autocomplete("option","source",s3_gis_ac_set_source(2));
...
function s3_gis_zoomMap(a,b,c,f){S3.gis.mapWin.rendered&&(b=new OpenLayers.LonLat(a,b),f=new OpenLayers.LonLat(c,f),b.transform(S3.gis.proj4326,S3.gis.projection_current),f.transform(S3.gis.proj4326,S3.gis.projection_current),a=b.lon,b=b.lat,c=f.lon,f=f.lat);c=OpenLayers.Bounds.fromArray([a,b,c,f]);a=c.getCenterLonLat();c=map.getZoomForExtent(c,!0);16<c&&(c=16);S3.gis.mapWin.rendered?(map.setCenter(a),map.zoomTo(c)):(a.transform(S3.gis.proj4326,S3.gis.projection_current),S3.gis.mapPanel.center=a,
S3.gis.mapPanel.zoom=c)}function s3_gis_hide_selector(){$(".locselect").hide()}function s3_gis_hide_selector_all(){$("#gis_location_label_row").hide();$(".subheading").hide();s3_gis_hide_selector()}function s3_gis_show_selector(){$("#gis_location_label_row").show();$(".subheading").show();s3_gis_show_tab("add")}
...
function s3_gis_select_search_result(a){a=$("body").data("id");$("#"+S3.gis.location_id).val(a);$("#gis_loc_add_tab").hide();$("#gis_loc_search_tab").hide();$("#gis_loc_view_tab").removeClass("hide").show();$("#gis_loc_edit_tab").removeClass("hide").show();s3_gis_hide_search_fields();a=$("#gis_location_name_search").val();$("#gis_location_name").val(a);a=$("#gis_location_street_search").val();$("#gis_location_street").val(a);a=$("#gis_location_postcode_search").val();$("#gis_location_postcode").val(a);
